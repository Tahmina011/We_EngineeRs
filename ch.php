...
function showResult(str) {
  ...
  xmlhttp.onreadystatechange=function() {
    if (this.readyState==4 && this.status==200) {
      ...
      document.getElementById("check").style.border="1px solid #A5ACB2";
    }
  }
  xmlhttp.open("POST","rating.php?user="+str,true);
  xmlhttp.send();
}
</script>
<?php
...
<div class="row" style="background-color: #E7EBEA;">
	...
	<div class="column middle">
		...
		while($ro=$res->fetch_assoc())
		{
			...
			$u=$ro['username'];
			echo '<form>
		<input type="submit" name="Right" value="Right" onclick="showResult('.$u.')">
		<input type="submit" name="Wrong" value="Wrong" onclick="showResult('.$u.')">
	</form>';
	echo "<div id='check' style='color:black;'>num:</div>";
	echo '</div>';
		}
