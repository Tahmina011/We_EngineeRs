<?php 
  $conn = mysqli_connect('localhost', 'root', '', 'achi');
  if (!$conn) {
  ...
  if (isset($_POST['save'])) {
    ...
    $post_id=$_POST['post_id'];
      ...
      $saved_comment = '<div class="comment_box">
      		...
      		<div class="display_name">'. $name .'</div><br>
  ...
  if (isset($_POST['update'])) {
  	...
  	if (mysqli_query($conn, $sql)) {
  		$id = mysqli_insert_id($conn);
  		$saved_comment = '<div class="comment_box">
  		  <span id="delete" data-id="' . $id . '" >delete</span>
...
function fetch_comment()
{
  ...
  while ($row = mysqli_fetch_array($result)) {
  	$comments .= '<div class="comment_box">
  		  ...
  		  <div class="comment_text">'.nl2br($row['comment']) .'</div>
  	  </div>';
  }
  $comments .= '</div>';
  return $comments;
